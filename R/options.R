...
#' @param stream_workers,normalize_workers,metric_workers Stage worker limits.
...
lidar_options <- function(block_m = 500, buffer_m = 60,
                          ...
                          stream_workers = 8, normalize_workers = 48,
  ...
  workers <- c(stream_workers = stream_workers, normalize_workers = normalize_workers, metric_workers = metric_workers)
  ...
  list(block_m = as.numeric(block_m), buffer_m = as.numeric(buffer_m), families = families,
       stream_workers = as.integer(stream_workers), normalize_workers = as.integer(normalize_workers),
